...
- **Per-endpoint outbox gauges** — `IOutboxMetricsQuery` is endpoint-agnostic by design (the SQL Server outbox is one table across all endpoints), so the `nimbus.outbox.pending` gauge is currently a single global series. If per-endpoint breakdowns become a requirement, the contract needs an `endpointId` parameter and the SQL Server schema needs a denormalised `EndpointId` column. The spec's FR-041 row for `nimbus.outbox.pending` / `dispatch_lag` lists `nimbus.endpoint` as an attribute; until the contract change lands, the shipped gauges emit the series without that tag.
- **SC-007 / NFR-001 — ≤5% overhead at p99 benchmark.** No `BenchmarkDotNet` project shipped; allocation-level inspection of the hot paths (publisher decorator, consumer middleware, outbox dispatcher) shows no obvious hot-path allocations that would breach the budget, but this is not measured. A `tests/NimBus.OpenTelemetry.Benchmarks/` project is the natural home, owned by the Phase 4.3 polish work.
- **FR-002 / NFR-003 — assembly-reference deny-list test.** The csproj at `src/NimBus.OpenTelemetry/NimBus.OpenTelemetry.csproj` lists only `OpenTelemetry.Api`, `OpenTelemetry.Api.ProviderBuilderExtensions`, and `OpenTelemetry.Extensions.Hosting`. Static enforcement (a reflection test or arch-fitness check) would lock that in; ship with 4.3.
- **FR-006 — degraded mode when `Services.AddNimBusInstrumentation` is not called.** Provider-builder extensions work standalone (`AddMeter` / `AddSource` only); decorators and the gauge service simply aren't registered. The contract is the spec's; an explicit test (drive `MeterProviderBuilder.AddNimBusInstrumentation()` alone, observe `NimBusMeters.MessagesPublished.Add(1)` surfaces in the exporter) is a doc-grade test deferred with 4.3 documentation.
- **FR-008 — canonical three-call wiring documentation.** Pattern is shown in `ServiceDefaults.Extensions.cs:45-61` and is the de-facto canonical wiring inside the repo. The dedicated `docs/observability.md` walkthrough is part of the Phase 4.3 documentation bucket (FR-090).
- **FR-012 — span lifetime covers broker settle.** `MetricsMiddleware` (`Pipeline/MetricsMiddleware.cs`) wraps the entire pipeline including downstream behaviors that perform settle calls, so the span end naturally follows settle. A dedicated end-to-end test that asserts span duration ≥ a forced settle delay is a hardening item for 4.3.
- **FR-020 `messaging.message.body.size`, `messaging.operation.name`, `messaging.destination.template`.** Constants present on `MessagingAttributes` but not yet set on any span. The body-size capture in particular wants the raw broker-side serialized length, which is most naturally added at the transport boundary — folded into the Phase 4.3 attribute-coverage hardening alongside the consumer-side `messaging.system` work.
- **FR-021 `nimbus.permanent_failure`, `nimbus.delivery_count`, `nimbus.handler.type`.** Constants present on `MessagingAttributes`; not yet wired on the consumer span. Same 4.3 attribute-coverage bucket as the FR-020 items.
- **FR-023 / FR-024 — Service Bus transport-specific attributes.** `messaging.servicebus.message.delivery_count`, `enqueued_time`, `destination.subscription_name`, and `message.session.id` constants are declared on `MessagingAttributes` but not yet set by `ServiceBusAdapter` / `MetricsMiddleware`. Folded into the same 4.3 attribute-coverage bucket.
- **FR-033 — `Baggage` passthrough.** The instrumentation never reads or writes `Baggage`, satisfying the MUST. A dedicated passthrough test (set `Baggage.Current["k"]="v"`, publish, assert key absence in span tags + value survives round-trip) is a hardening item for 4.3.
- **FR-043 — exponential-bucket histogram aggregation.** `AddNimBusInstrumentation` does not configure `ExplicitBucketHistogramConfiguration` for any instrument, so consumer-side OTel SDK defaults apply (exponential where supported). An assertion test is a hardening item for 4.3.
- **NFR-002 / NFR-007 / NFR-008 / NFR-009.** Allocation-light registration, host-shape portability (console / ASP.NET Core / Azure Functions), no-`ILogger` invariant in decorator hot paths, and verbose-mode parent-traceparent preservation. All are quality invariants of the shipped code; dedicated assertion tests deferred with the 4.3 polish bucket.
