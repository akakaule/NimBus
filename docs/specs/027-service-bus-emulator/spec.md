...
- **SES-4** — Explicit accept of session `S` is a **conditional wait**, matching [Azure's model](https://learn.microsoft.com/en-us/azure/service-bus-messaging/message-sessions) where a session materializes with its first message:
  ...
  - `S` **never materialized** (no messages, no state): **pend the attach** — do not complete it — until a message for `S` is enqueued or its state is set (→ complete with the lock) or the client detaches/cancels. There is **no broker-side timeout for explicit accepts**: the SDK sends the `com.microsoft:timeout` link property only for *next-available* accepts; explicit accepts are cancelled **client-side** (`PublisherClient.cs:267` passes a local cancellation), so the broker's job is simply to hold the pending attach until resolution or detach, and to clean up the waiter on detach. **M2 live-probe task**: run the never-materialized accept against real Azure and record the exact exception the SDK surfaces on client-side cancellation/timeout; pin that in the `[CommonFidelity]` test rather than guessing. This pend-until-materialized behavior is load-bearing for request/reply: `PublisherClient` sends the request and accepts its unique reply session *before the reply necessarily exists* (`PublisherClient.cs:257,276`) — the accept must stay pending, then complete when the reply is published.
...
| **M2** | Sessions (explicit + next-available + state + locks incl. umbrella model), `$management` ops **A/B/C/E**, scheduled messages, TTL, delivery counts, DLQ, status enforcement, error mapping table; SES-4 Azure live-probe | Named `[CommonFidelity]` tests green: session-processor ordering, umbrella-lock long-handler, schedule/cancel + **TTL-starts-at-activation**, exact delivery counts (SES-9), max-delivery → DLQ (SET-3), status-flip guards, error-mapping table; ASP-5 verified |
