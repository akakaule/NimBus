...
| Dispatch failed | the same span ends `nimbus.outcome=failed` with `error.type`, and the `nimbus.outbox.dispatched` counter records the failed outcome. A send that outran its lease budget adds a `nimbus.outbox.send_budget_expired` event — that is a **failed attempt**, distinct from host shutdown, which records nothing and leaves the row pending | `Outbox dispatch exceeded its send budget for message {OutboxId} …` (Warning) or `Outbox dispatch failed for message {OutboxId} …` (Error) |
