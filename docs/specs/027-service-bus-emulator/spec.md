...
- **SEC-3** — Connection/frame bounds, concrete defaults (all configurable): ≤ 256 concurrent TCP connections (refused beyond, never queued unboundedly); AMQP `max-frame-size` 1 MiB; ≤ 16 AMQP channels (sessions) per connection; ≤ **2 048 links per connection** — sized for the Functions Resolver's `maxConcurrentSessions: 200` (each active session ≈ one receive link + management links) with an order of magnitude of headroom.
- **SEC-5** — Protocol strictness and deadlines: the multiplexer classifies on a **bounded exact prefix**, not one byte — AMQP requires the full 8-byte protocol header (`AMQP` + version bytes), HTTP requires a token from the known method set (`GET PUT POST DELETE HEAD OPTIONS PATCH`) followed by a space; anything else → close immediately. Deadlines, all enforced server-side: 5 s from accept to a classified prefix, 10 s to complete SASL+open, 10 s for HTTP request headers, 60 s idle timeout on connections with no open links and no in-flight requests. Slow-loris and garbage-prefix cases are `[EmulatorOnly]` tests alongside SEC-2/SEC-3 malformed-input cases.
...
- **SES-4** — Explicit accept of session `S` is a **conditional wait**, matching [Azure's model](https://learn.microsoft.com/en-us/azure/service-bus-messaging/message-sessions) where a session materializes with its first message:
  - `S` **unlocked and materialized** (≥1 message in any state, or non-empty session state): complete the attach immediately with the lock.
  - `S` **locked by another receiver**: wait up to the client's timeout for release; still locked → reject attach with `com.microsoft:session-cannot-be-locked` (→ `SessionCannotBeLocked`). This contention case is how the five NimBus catch-sites actually encounter the error (e.g. `DeferredMessageProcessor` accepting a session the endpoint's own processor currently holds).
  - `S` **never materialized** (no messages, no state): **pend the attach** — do not complete it — until a message for `S` is enqueued or its state is set (→ complete with the lock), the client's timeout elapses (→ reject with `com.microsoft:timeout` → `ServiceTimeout`), or the client detaches/cancels. This is load-bearing for request/reply: `PublisherClient` sends the request and accepts its unique reply session *before the reply necessarily exists* (`PublisherClient.cs:257,276`) — the accept must stay pending, then complete when the reply is published.
...
- **BRK-6a** — Forward transfer protocol (all transitions mediated by the **source** actor, which is the sole writer of its own store):
  1. **Reserve**: the source actor moves the head message `Active → TransferPending`, stamps it with the pump's current **generation** (see below), and posts a transfer request to the target actor's mailbox. `TransferPending` messages are invisible to peeks-for-delivery and never receivable (receive is rejected on forwarding subscriptions anyway, LNK-4).
  2. **Commit**: the target actor enqueues the copy through its full pipeline and posts an acknowledgement back to the source actor's mailbox; on receiving it, the source actor removes the message (`TransferPending → Forwarded`, i.e. gone). Only then is the next message reserved — transfers are strictly one-at-a-time per subscription, preserving order.
  3. **Fault**: the target actor reports hop-limit exceeded / entity missing / `SendDisabled` instead of acking; the source actor moves the message `TransferPending → TransferDLQ`.
...
- **STO-4** — Restart semantics (in-memory default). An emulator restart inside a running AppHost **loses all topology and messages by design**, and the one-shot provisioner does not rerun on its own — so a restarted emulator must not silently look healthy-and-provisioned:
  - `/health` includes an `instance` id (fresh GUID per process start) so tooling and tests can detect a restart behind an otherwise-green probe.
  - Supported recoveries, documented in the README: restart the **AppHost** (the default answer), or restart the **provisioner resource** from the Aspire dashboard — `ApplyAsync` provisions from empty idempotently.
  - `[EmulatorOnly]` test: kill and restart the emulator process, rerun the provisioner, assert full topology is restored and publish/consume works.
  - Where restart-resilience actually matters, the durable journal (STO-1/STO-2) is the opt-in answer — topology and messages replay on boot.
...
Implemented as a project-backed resource (no container, no image pulls, sub-second start). ⚠ `ProjectResource` does **not** implement `IResourceWithConnectionString` in Aspire 13.4.6 (verified by reflection during review), so the resource type must be explicit. Concrete contracts, since the hosting library cannot reference the AppHost's generated `Projects.*` metadata itself:
  - **Resource type**: a custom `NimBusServiceBusEmulatorResource : ProjectResource, IResourceWithConnectionString` whose `ConnectionStringExpression` is built from its own endpoint reference. If subclassing `ProjectResource` proves unworkable against the real Aspire APIs, the specified fallback is a **two-resource handle**: a struct exposing `.Project` (the `ProjectResource`) and `.ConnectionString` (a `ConnectionStringResource`), consumed as `.WithReference(servicebus.ConnectionString).WaitFor(servicebus.Project)`. **M4 must compile-test the chosen shape as its first task** — the exact `Program.cs` snippet in this section is not considered specified until it compiles.
...
- **TST-3 — Dual-target compatibility runs.** The `[CommonFidelity]` subset runs against a **real Azure namespace** (`NIMBUS_SBEMULATOR_COMPAT_CS`). Divergence between targets is a red build. Because the existing CI workflow carries no Azure credential (`.github/workflows/dotnet.yml`), env-gating alone would let the gate **silently skip** — so the operating rules are explicit:
  - A dedicated `servicebus-emulator-compat` **manual/post-merge workflow** (`workflow_dispatch` + `push` to master) holds the connection string as a repo secret; it is **never** exposed to `pull_request` runs of untrusted code.
...
- **TST-4 — AppHost e2e.** Two smoke levels, both M4:
  ...
  - **Main `src/NimBus.AppHost`** in emulator mode: the **Functions-based Resolver** (`[ServiceBusTrigger]`, `maxConcurrentSessions: 200` — a different client stack than the SDK hosts) processes a message end-to-end; the **CLI** runs `nb topology apply` (zero-churn second run) and a `purge-subscription`; and the WebApp subscription-admin flows (pause/resume/purge/rebuild) are driven **through the WebApp's HTTP API surface** (`AdminImplementation` routes), not just `IServiceBusManagement` in-process.
