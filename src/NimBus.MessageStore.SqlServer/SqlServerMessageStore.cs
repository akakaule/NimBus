  ...
  AND (QueueTimeMs IS NOT NULL OR ProcessingTimeMs IS NOT NULL)";
    ...
    public async Task<EventMapping?> GetMapping(string id)
    {
        ...
        var row = await conn.QuerySingleOrDefaultAsync(
            $"SELECT * FROM {T("EventMappings")} WHERE [Id] = @id",
            new { id },
            commandTimeout: _commandTimeout);
        return row == null ? null : MapMappingRow(row);
    }
    ...
    public async Task<EventMapping?> GetActiveMappingForSource(string sourceEventTypeId)
    {
        await using var conn = Open();
        var row = await conn.QueryFirstOrDefaultAsync(
            $"SELECT TOP 1 * FROM {T("EventMappings")} WHERE [SourceEventTypeId] = @src AND [State] = @state",
            new { src = sourceEventTypeId, state = MappingState.Active.ToString() },
            commandTimeout: _commandTimeout);
        return row == null ? null : MapMappingRow(row);
    }
    ...
    public async Task<EventMapping> SaveMapping(EventMapping mapping)
    {
        ...
        var sql = $@"
MERGE {T("EventMappings")} AS target
...
WHEN MATCHED THEN UPDATE SET
    ...
    Transform          = @Transform,
    Rationale          = @Rationale,
    WorkedExamplesJson = @WorkedExamplesJson,
...
WHEN NOT MATCHED THEN INSERT (
    Id, SourceEventTypeId, TargetEventTypeId, Transform, Rationale, WorkedExamplesJson,
    SourceSchemaHash, State, Version, CreatedBy, CreatedUtc, ApprovedBy, ApprovedUtc)
VALUES (
    @Id, @SourceEventTypeId, @TargetEventTypeId, @Transform, @Rationale, @WorkedExamplesJson,
    @SourceSchemaHash, @State, @Version, @CreatedBy, @CreatedUtc, @ApprovedBy, @ApprovedUtc);";
        ...
        await conn.ExecuteAsync(sql, new
        {
            ...
            mapping.Transform,
            mapping.Rationale,
            mapping.WorkedExamplesJson,
            mapping.SourceSchemaHash,
            State = mapping.State.ToString(),
            mapping.Version,
            mapping.CreatedBy,
            mapping.CreatedUtc,
            mapping.ApprovedBy,
    ...
    private static EventMapping MapMappingRow(dynamic row) => new()
    {
        Id = row.Id,
        ...
        WorkedExamplesJson = row.WorkedExamplesJson,
        SourceSchemaHash = row.SourceSchemaHash,
        State = Enum.TryParse((string)row.State, out MappingState ms) ? ms : MappingState.Draft,
        Version = row.Version,
        CreatedBy = row.CreatedBy,
        CreatedUtc = row.CreatedUtc,
        ApprovedBy = row.ApprovedBy,
        ApprovedUtc = row.ApprovedUtc is DateTime dt ? dt : (DateTime?)null,
    };
}
