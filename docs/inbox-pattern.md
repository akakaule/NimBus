...
SQL Server stores records in an independently provisioned inbox table with a **nonclustered** composite primary key on `(EndpointId, MessageId)` — the key can reach 1,544 bytes, past the 900-byte clustered-key limit — and a clustered index on the creation timestamp for efficient bounded purges. The purge uses `READPAST` together with `READCOMMITTEDLOCK`, so it also works on databases with `READ_COMMITTED_SNAPSHOT` enabled (the Azure SQL default). Cosmos DB stores inbox documents in a dedicated container keyed by a hash of the endpoint and message identity, uses explicit bounded cleanup with ETag-conditioned deletes (a record recreated between the purge query and the delete survives), and does not depend on container TTL. The in-memory provider keeps timestamps in a concurrent map.
