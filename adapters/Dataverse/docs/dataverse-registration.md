...
6. Configure allowed column projections through `Dataverse__Tables__<logical-name>__<index>`; each allowlisted table needs at least one column. Projected values reach the WebApp masked for operators without the PiiReader role. Set OrganizationId to the source organization GUID, not its URL or tenant ID.
