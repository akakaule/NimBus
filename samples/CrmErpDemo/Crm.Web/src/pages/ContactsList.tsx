  ...
  return (
    <section>
      <header className="flex items-center mb-4">
        ...
        <Link to="/contacts/new" className="ml-auto px-3 py-2 bg-blue-600 text-white rounded-md text-sm">New contact</Link>
      </header>
          ...
          <tbody>
            ...
            {rows.map(r => (
              <tr key={r.id} className={`border-t border-slate-100 ${r.isDeleted ? 'opacity-60 line-through' : ''}`}>
                <td className="px-4 py-2"><Link to={`/contacts/${r.id}`} className="text-blue-700 hover:underline">{r.firstName} {r.lastName}</Link></td>
                <td className="px-4 py-2 text-slate-500">{r.email ?? '—'}</td>
                <td className="px-4 py-2 text-slate-500">{r.phone ?? '—'}</td>
                <td className="px-4 py-2 text-slate-500">{r.accountId ?? '—'}</td>
