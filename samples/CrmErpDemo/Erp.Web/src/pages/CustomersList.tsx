...
export default function CustomersList() {
  ...
  async function load(initial = false) {
    ...
    try { setRows(await api.listCustomers()); } finally { if (initial) setLoading(false); }
  }
  useEffect(() => {
    load(true);
  ...
  return (
    <section>
      <header className="flex items-center mb-4">
        ...
        <Link to="/customers/new" className="ml-auto px-3 py-2 bg-emerald-600 text-white rounded-md text-sm">New customer</Link>
      </header>
          ...
          <tbody>
            ...
            {rows.map(r => (
              <tr key={r.id} className={`border-t border-slate-100 ${r.isDeleted ? 'opacity-60 line-through' : ''}`}>
                ...
                <td className="px-4 py-2"><Link to={`/customers/${r.id}`} className="text-emerald-700 hover:underline">{r.legalName}</Link></td>
