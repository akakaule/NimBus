...
- **SES-4** — Explicit accept of session `S` is a **conditional wait**, matching [Azure's model](https://learn.microsoft.com/en-us/azure/service-bus-messaging/message-sessions) where a session materializes with its first message:
  ...
  - **Waiter arbitration is atomic**: materialization, waiter selection, and session-lock acquisition are **one broker-core transition** — at most one pending accept (explicit or next-available) resolves successfully per session-ownership generation. Losing waiters stay pending under their own timeout rules (an explicit waiter now contends per the locked-session branch; a next-available waiter goes back to watching all sessions). FIFO fairness among waiters is not promised; single ownership is normative. The same rule arbitrates several `AcceptNextSessionAsync` waiters racing for one newly available session.
